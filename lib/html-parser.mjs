...
const document = {
  ...
  querySelector() { return document },
  querySelectorAll() { return [document] },
}
...
container._legoRegistry = container._legoRegistry || new Map()
...
export default class HTMLParser {
  ...
  */
      ...
      Array.from(node.attributes).forEach((attr) => {
        if(ATTR_REGEX.test(attr.value)) {
          ...
          if (isPrimitive(value)) node.setAttribute(attr.name, value)
            ...
            container._legoRegistry.set(md5(value), value)
  ...
  static evaluateString(str, context) {
    ...
    const templater = new Function(`return eval(\`${(container._legoRegistry.get(str) || str)}\`)`)
    return templater.call(context) || ''
  }
